...
import { useTheme } from '@/context/ThemeContext'; // Import theme consumer
import { useRouter } from 'next/navigation';
import Link from 'next/link';

export default function LoginPage() {
    const { login, loginWithGoogle } = useAuth();
    const { theme } = useTheme(); // Consume global theme variable
    ...
    // Quick Demo Login configuration mapper
    // Quick Demo Login configuration mapper
    const demoAccounts = [
        { 
            role: 'Admin', 
    ...
    const handleQuickLogin = (email) => {
        // Automatically fill inputs using corresponding credentials configuration 
        setFormData({
            email: email,
            password: 'password123' // Fallback matching common development testing string
        });
        setError('');
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            ...
            setError(result.message || 'Invalid credentials combination.');
            setSubmitting(false);
        }
    };
    ...
    return (
        /* Outer Background Wrapper transitions adaptively */
        <main className={`flex min-h-screen items-center justify-center px-4 py-12 font-sans select-none transition-colors duration-200 ${
            ...
            {/* Card Content block changes colors smoothly */}
            <div className={`w-full max-w-[440px] p-8 border rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.15)] transition-colors duration-200 ${
                theme === 'dark' 
                ...
                <div className="mb-8 text-center">
                    ...
                    <h1 className={`mt-4 text-2xl font-bold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>Welcome back</h1>
                    <p className={`mt-1 text-xs ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>Sign in to your account</p>
                </div>
                ...
                {/* Quick Demo Login Section Container layout */}
                <div className={`mb-6 p-4 rounded-xl border transition-colors duration-150 ${
                    theme === 'dark' ? 'bg-[#18181c] border-neutral-800/60' : 'bg-neutral-50 border-neutral-200/80'
                }`}>
                    <h2 className={`text-[11px] font-semibold tracking-wider uppercase mb-3 ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>Quick Demo Login</h2>
                    <div className="space-y-2.5">
                        {demoAccounts.map((account) => (
                            <button
                                ...
                                className={`w-full flex items-center text-left text-xs border px-3 py-2 rounded-lg transition-all duration-150 group ${
                                    theme === 'dark'
                                        ? 'bg-[#121214] hover:bg-neutral-800/40 border-neutral-800'
                                        : 'bg-white hover:bg-neutral-100 border-neutral-200'
                                }`}
                            >
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded border transition-colors duration-150 mr-3 w-14 text-center ${account.color}`}>
                                    {account.role}
                                </span>
                                <span className={`font-mono text-[11px] ${
                                    theme === 'dark' ? 'text-neutral-400 group-hover:text-neutral-200' : 'text-neutral-600 group-hover:text-neutral-900'
                                }`}>
                                    {account.email}
                                </span>
                            </button>
                ...
                {/* Traditional Interaction Gates Form */}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className={`block text-xs font-medium mb-1.5 ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>Email</label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className={`w-full border pl-10 pr-4 py-2.5 text-xs rounded-lg focus:border-[#2563eb] focus:outline-none focus:ring-1 focus:ring-[#2563eb] transition-all ${
                                    theme === 'dark' 
                ...
                {/* Google Authentication Integration Route */}
                <button
                ...
                <p className={`mt-6 text-center text-xs ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>
                    ...
                    <Link href="/register" className="font-semibold text-[#2563eb] hover:underline underline-offset-4">
                        Create one 
                    </Link>
                </p>
            </div>
